...
export default async function (client: UMClient) {
    ...
    const commandFolders = await fs.readdirSync(`${process.cwd()}/commands`);

    for (let commandFolder of commandFolders) {
        const commandFiles = await fs.readdirSync(`${process.cwd()}/commands/${commandFolder}`).filter(file => file.endsWith('.js'));
